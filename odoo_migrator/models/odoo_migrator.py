...
class OdooMigrator(models.Model):
    ...
    def migrate_account_moves_lines(self, move_type: str = "") -> bool:
        ...
        for migrator in self:
            ...
            for contador, move_line_data in enumerate(move_line_datas, start=1):
                ...
                is_success, result = migrator.try_to_create_record(
                    odoo_object=move_line_obj, value=move_line_data, old_odoo_obj=model_name_old
                )
    ...
    def migrate_payments_account_moves_lines(self, payment_type: str = "") -> bool:
        ...
        if bool(payment_type):
            account_payments_ids = account_payments_ids.filtered(lambda payment: payment.partner_type == payment_type and payment.state != 'posted')
        model_name: str = "account.move.line"
        model_name_old: str = "account.move.line"
        ...
        for migrator in self:
            ...
            if not bool(move_line_datas):
                continue
            payment_currency_dict = {item['move_id'][0]: item['payment_id'][0] for item in move_line_datas if item['move_id'] and item['payment_id']}
            ...
            for contador, move_line_data in enumerate(move_line_datas, start=1):
                print(f"vamos {contador} / {total}")
                payment_id = move_line_data.get("payment_id")[0] if move_line_data.get("payment_id", False) else payment_currency_dict[move_line_data.get("move_id", False)[0]]
                ...
                if not account_line:
                    ...
                    dic_moves[move] = 'Error en una linea'
                    continue
                amount_currency = (move_line_data.get('balance')) if company_currency else move_line_data.get('amount_currency')
                dic_move_line = {
            ...
            filtered_moves = {key: value for key, value in dic_moves.items() if value != 'Error en una linea'}
