...
PRODUCT_PRODUCT_FIELDS: List[str] = [
    "id",
    "active",
    "name",
    "sale_ok",
...
class OdooMigrator(models.Model):
    ...
    product_templates_ids = fields.Many2many(
        comodel_name="product.template", string="productos"
    )
    ...
    product_product_count = fields.Integer(string="Número de ", compute='_get_models_data_count')
    ...
    def _get_models_data_count(self):
        for rec in self:
            ...
            rec.product_product_count = len(rec.products_ids)
    ...
    def action_view_model(self):
        ...
        elif model_name == 'product_product':
            action = self.env.ref("account.product_product_action_sellable").read()[0]
            action['context'] = {}
            action["domain"] = [("id", "in", self.products_ids.ids)]
        ...
        elif model_name == 'entry_reconcile':
            action = self.env.ref("contacts.action_contacts").read()[0]
            action["domain"] = [("id", "in", self.contact_ids.ids)]
    ...
    def update_account_account(self):
        """
        Para cuando hay muchas cuentas contables repetidas actualizamos agregamos un ".".
        """
        _logger.info("\nActualizamos Actual Plan de cuenta")
        model_name: str = "account.account"
        ...
        for migrator in self:
            ...
            if not conflictive_accounts:
                raise UserError(f"No hay cuentas para actualizar")
            for account in chart_of_accounts_obj.search([('code', 'in', conflictive_accounts)]):
                account_reconcile = account.reconcile
                code = account.code + '.'
                account.write({'code': code, 'reconcile': account_reconcile})
    ...
    def test_connection(self):
        ...
        for migrator in self:
            ...
            self.source_version = version.split('-')[0]
    ...
    def create_odoo_record(self, m2oid=None, value=None, odoo_object=None, lang=None):
        ...
        if odoo_object._name == "res.partner":
            ...
            elif odoo_object._name == 'product.template':
                odoo_object_required_fields = PRODUCT_FIELDS
        ...
        if odoo_object._name in ["res.country", "res.country.state"]:
            domain.insert(0, "|")
            domain.append(("code", "=", value["code"]))
        if odoo_object._name == 'product.template':
            record = False
        else:
            record = odoo_object.with_context(default_lang=lang, lang=lang).search(domain, limit=1)
        if not record:
            if self._context.get('from_company') and odoo_object._name == 'res.partner':
                value.pop('child_ids')
            try:
    ...
    def resolve_m2o_fields(self, value=None, odoo_object=None, m2o=None, lang=None):
        ...
        try:
            ...
            if m2o != 'parent_id' or odoo_object._name != 'product.category':
                odoo_object = self.env[field_to_model.get(m2o)].with_context(lang=lang)
        except Exception as error:

            raise UserError(error)
        try:
            if m2o == 'product_tmpl_id':
                domain = [("old_id", "=", record_old_id)]
            else:
    ...
    def try_to_create_record(self, odoo_object=None, value=None, old_odoo_obj=None):
        ...
        try:
            ...
            if not self._context.get('check_move_validity', True):
                record = odoo_object.sudo().with_context(check_move_validity=False, from_migrator=True).create(value)
            else:
                record = odoo_object.sudo().with_context(check_move_validity=False, from_migrator=True).create(value)
            return True, record
        except Exception as error:
    ...
    def _remove_m2o_o2m_and_m2m_data_from(self, data, model_obj=None, lang=None):
        ...
        for field in list(data):
            ...
            if field_type == "many2one" and is_field_empty:
                if model_obj._name == 'product.category' and field == 'parent_id':
                    data = self.resolve_m2o_fields(value=data, odoo_object=model_obj, m2o=field, lang=lang)
                else:
            ...
            for contador, analytic_accounts_data in enumerate(analytic_accounts_datas, start=1):
                ...
                if not bool(analytic_account_id):
                    ...
                    if not is_success:
                        ...
                        continue
                    elif not analytic_account_active:
                        result.active = False
    ...
    def migrate_product_categories(self) -> bool:
        ...
        for migrator in self:
            ...
            for contador, product_category_data in enumerate(product_category_datas, start=1):
                ...
                if bool(product_category_id) and category_name != 'Fixed Assets':
    ...
    def migrate_product_template(self) -> bool:
        """
        Método para migrar las productos desde el Odoo de origen al Odoo de destino.
        """
        _logger.info("\nMigrando Productos")
        self = self.sudo()
        ...
        for migrator in self:
            for categ_id in migrator.product_categories_ids:
                _logger.info(f"migrando Productos para la Categoria {categ_id.display_name}")
                domain = ['&', ('categ_id', '=', categ_id.old_id), '|', ('active', '=', True), ('active', '=', False)]
                operation_params_list = domain
                product_product_datas = migrator._run_remote_command_for(
                    model_name=model_name,
                ...
                if not bool(product_product_datas):
                    continue

                total = len(product_product_datas)
                commit_count = 5
                side_count = 0
                for contador, product_product_data in enumerate(product_product_datas, start=1):
                    _logger.info(f"vamos {contador} / {total}")
                    side_count += 1
                    if side_count >= commit_count:
                        ...
                        self.env.cr.commit()
                        side_count = 0
                    product_old_id = product_product_data["id"]
                    template_name = product_product_data["name"]
                    _logger.info(template_name)
                    product_domain = ['&', ('old_id', '=', product_old_id), '|', ('active', '=', True), ('active', '=', False)]
                    product_product_id = product_product_obj.search(product_domain,limit=1)

                    if bool(product_product_id):
                        _logger.info(f"la Plantilla de producto {template_name} ya existe")
                        product_product_id.old_id = product_old_id
                        product_product_id.categ_id = categ_id.id
                        product_product_id.company_id = self.company_id.id
                        migrator.products_ids += product_product_id
                        continue
                    product_template_old_id = product_product_data["product_tmpl_id"][0]
                    product_template = product_template_obj.search([('old_id', '=', product_template_old_id)])
                    migrator._remove_m2o_o2m_and_m2m_data_from(data=product_product_data, model_obj=product_product_obj)

                    if not product_template:
                        product_template = product_template_obj.browse(product_product_data.get('product_tmpl_id'))
                        product_product = product_template.product_variant_id
                        product_product.old_id = product_old_id
                        product_product_id.categ_id = categ_id.id
                        product_product_id.company_id = self.company_id.id
                        migrator.products_ids += product_product_id
                        migrator.create_success_log(values=product_product_data)
                        _logger.info(f"se creo el producto ->> {product_product_id.name}")
                        continue
                    else:
                        is_success, result = migrator.try_to_create_record(odoo_object=product_product_obj, value=product_product_data)

                    if not is_success:
                        migrator.create_error_log(msg=str(result), values=product_product_data)
                        continue
                    if not result:
                        result.product_tmpl_id.old_id = result.old_id
                        result.categ_id = categ_id.id
                    migrator.products_ids += result
                    migrator.create_success_log(values=product_product_data)
                    _logger.info(f"se creo el producto ->> {result.name}")

            _logger.info(
                f"se crearon {len(self.products_ids)} productos"
            )
        return True
    ...
    def migrate_account_moves(self, move_type: str = "") -> bool:
        ...
        if move_type == 'entry':
            ...
            old_journal_ids = self.journal_ids.filtered(lambda x: x.company_id == self.company_id).mapped("old_id")
            operation_params_list = [
        ...
        for migrator in self:
            account_move_datas = migrator._run_remote_command_for(
                ...
                command_params_dict={
                    ...
                    "offset": migrator.pagination_offset,
                    "limit": migrator.pagination_limit,
                    "order": "date asc" if move_type == "entry" else "invoice_date asc",
                },
            )
            ...
            for contador, account_move_data in enumerate(account_move_datas, start=1):
                ...
                if move_type != "entry":
                    ...
                    account_move_data["is_manual_entry"] = True
    ...
    def post_moves(self, move_type: str = "", payment_type: str = "") -> bool:
        ...
        if self.migration_model == 'post_supplier_payments':
            repeat_names = ['Ahmad taxi to collect money from Deira',
                            ...
                            'NBDEU/2024/0003',
                            'Passport Photo size ',
                            'PASSPORT PHOTOS FOR A CLIENT',
                            'Wifi airport',
                            '02.09_MOHAMED DERBAL ll BATCH 1 - TRANSLATION',
                            '02.09_URGENT Request for ENGLISH TRANSLATION MR. ABDELKRIM MAGHNIA - BATCH 2 ( 7 DOCUMENTS)',
                            '19.09_MRS.HELANA MARIE SARKIS ll TRANLSATION -BATCH 1',
                            '27.09_FOR URGENT TRANSLATION -MAHDI MASOUMI- BATCH 1',
                            '10.10_HANIEH MIRBAHAI ll 5 DOCUMENTS FOR ENGLISH TRANSLATION',
                            '18.10_MR. NASSER DIAB - BATCH 7 (5 DOCUMENTS)',
                            '18.10_HUSSEIN YOSRI SHERIF HUSSEIN YOSRI GOHAR- FOR ENGLISH TRANSLATION',
                            '28.10_Joe Abi Akl II Docs for English Translation',
                            '03.11__TALAL MAHMOOD BELEED AMIR ll TRANSLATION ll BATCH 1',
                            '23.11_TALAL MAHMOOD BELEED AMIR ll TRANSLATION ll BATCH 2']
            for move in self.env['account.move'].search(['|', ('name', 'in', repeat_names), ('old_name', 'in', repeat_names), ('state', '=', 'draft')]):
                unique_name = f"{move.name}-{move.id}"

                if move.old_name not in repeat_names:
                    if unique_name != move.name:
                        move.name = move.old_name
                    if unique_name != move.payment_id.name:
                        move.payment_id.name = move.old_name
                else:
                    if unique_name != move.name:
                        move.name = unique_name
                    if unique_name != move.payment_id.name:
                        move.payment_id.name = unique_name
            self.env.cr.commit()
            for move in self.env['account.payment'].search(['|', ('name', 'in', repeat_names), ('old_name', 'in', repeat_names), ('state', '=', 'draft')]):
                unique_name = f"{move.old_name}-{move.id}"

                if move.old_name not in repeat_names:
                    if unique_name != move.name:
                        move.name = move.old_name
                    if unique_name != move.move_id.name:
                        move.move_id.name = move.old_name
                else:
                    if unique_name != move.name:
                        move.name = unique_name
                    if unique_name != move.move_id.name:
                        move.move_id.name = unique_name
            self.env.cr.commit()
        for contador, move in enumerate(moves_draft, start=1):
            ...
            if move.old_state == "draft":
                ...
                try:
                    ...
                    try:
                        if force_name:
                            move.move_id.name = force_name
                            move.name = force_name
                        elif name:
                            move.move_id.name = name
                        elif move.old_name and move.old_name not in repeat_names:
                            move.name = move.old_name
                    except Exception as error:
                        continue
                except Exception as error:
                    if "invalid literal for int() with base 10: 'false'" == str(error):
                        for line in move.line_ids.filtered(lambda x: x.analytic_distribution and x.analytic_distribution == {'false': 100.0}):
                            line_datas = self._run_remote_command_for(
                                model_name='account.move.line',
                                operation_params_list=[("id", "=", line.old_id)],
                                command_params_dict={
                                    "fields": ['analytic_account_id'],
                                    "offset": self.pagination_offset,
                                    "limit": self.pagination_limit,
                                })
                            if line_datas and line_datas[0].get('analytic_account_id', False):
                                analytic_id = aaa_obj.search([('old_id', '=', line_datas[0].get('analytic_account_id')[0])])
                                if not analytic_id:
                                    analytic_id = aaa_obj.search([('old_id', '=', line_datas[0].get('analytic_account_id')[0]), ('active', '=', False)])
                                if analytic_id:
                                    line.analytic_distribution = {analytic_id.id: 100}
                        try:
                            _logger.info(f'Vamos a validar la factura / pago {move.name} de id {move.id}')
                            move.with_context(dont_check_constrains_date_sequence=True).action_post()
                            if name:
                            ...
                            #
                            continue
                    elif "is not balanced" in str(error):
                        for line in move.line_ids.filtered(lambda x: x.display_type == "payment_term"):
                            line_datas = self._run_remote_command_for(
                                model_name='account.move.line',
                                operation_params_list=[("id", "=", line.old_id)],
                                command_params_dict={
                            ...
                            if line_datas:
                                if line_datas[0].get('credit', 0):
                                    line.credit = line_datas[0].get('credit')
                                else:
                                    line.debit = line_datas[0].get('debit')
                        if move.state != 'posted':
                            try:
                                ...
                                #
                                continue
                    elif 'duplicate key value violates unique constraint "account_move_unique_name"' in str(error):
                        self.env.cr.rollback()
                        if name:
                            move.move_id.name = f'{name}/{contador}'
                        else:
                            move.name = f'{move.old_name}/{contador}'
                        if move.state != 'posted':
                            try:
                                _logger.info(f'Vamos a validar la factura / pago {move.name} de id {move.id}')
                                move.with_context(dont_check_constrains_date_sequence=True).action_post()
    ...
    def migrate_invoice_account_moves_lines(self, move_type: str = "") -> bool:
        ...
        if not bool(self.account_moves_ids):
            raise UserError("No hay Asientos contables migrados")
        account_moves_ids = self.account_moves_ids.filtered(lambda move: not move.lines_migrated)
        if bool(move_type):
        ...
        account_moves_ids = account_moves_ids[:5000]
        model_name_old: str = "account.move.line"
        aaa_obj = self.env['account.analytic.account']
        aa_obj = self.env['account.account']
        am_obj = self.env['account.move']
        move_line_obj = self.env[model_name_old]
        operation_param = ("move_id", "in", account_moves_ids.mapped("old_id"))
        for migrator in self:
            ...
            for contador, move_line_data in enumerate(move_line_datas, start=1):
                ...
                if not move_line_data.get('currency_id'):
                    move_line_data['currency_id'] = company_currency
                if (move_line_data.get('product_id') and not move_line_data.get('display_type')) or not exclude_from_invoice_tab:
                    move_line_data['display_type'] = 'product'
                if move_line_data.get('display_type'):
                    move_line_data.pop('tax_line_id')
                    if move_id in move_dic:
                        if 'product' in move_dic[move_id]:
                            move_dic[move_id]['product'].append(move_line_data)
                        else:
            ...
            for move in move_dic:
                ...
                for product in lines_dic['product']:
                    ...
                    if old_analytic_data:
                        ...
                        if analytic_id:
                            product['analytic_distribution'] = {analytic_id.id: 100}
                    migrator._remove_m2o_o2m_and_m2m_data_from(data=product, model_obj=move_line_obj)
                    is_success, result = migrator.try_to_create_record(odoo_object=move_line_obj, value=product, old_odoo_obj=model_name_old)
                    if not is_success:
                        migrator.create_error_log(msg=str(result), values=product)
                    else:
    ...
    def migrate_account_move_lines(self, move_type: str = "") -> bool:
        ...
        journal_type = "general"
        # old_journal_ids = self.journal_ids.filtered(lambda x: x.type == journal_type).mapped("old_id")
        old_journal_ids = self.journal_ids.mapped("old_id")
        ...
        if move_type == "entry":
            ...
            if entries:
                operation_params_list = [('move_id', "in", entries.mapped('old_id'))]
    ...
    def migrate_account_payments(self, payment_type: str = "") -> bool:
        ...
        if bool(payment_type):
            if payment_type == 'internal_transfer':
                operation_params_list.append(("payment_type", "=", "transfer"))
            else:
        ...
        for migrator in self:
            ...
            for contador, payment_data in enumerate(payment_datas, start=1):
                ...
                if payment_type == 'internal_transfer':
                    ...
                    if not payment_data.get("partner_type", False):
                ...
                if not is_success:
                    ...
                    if result.is_internal_transfer:
                        result.move_id.old_id = 0
                        if move_name:
                            result.move_id.old_name = move_name
                    elif result.old_state == 'draft':
                        result.move_id.old_id = 0
    ...
    def migrate_payments_account_moves_lines(self, payment_type: str = "") -> bool:
        ...
        for migrator in self:
            ...
            for contador, move_line_data in enumerate(move_line_datas, start=1):
                ...
                move.old_id = move_line_data.get("move_id", False)[0]
    ...
    def migrate_reconcile_moves(self, move_type: str = "") -> bool:
        """
        ...
        for migrator in self:
            ...
            reconcile_datas = migrator._run_remote_command_for(
                model_name=model_name,
                operation_params_list=['|', ("debit_move_id", "in", move_ids), ("credit_move_id", "in", move_ids)],
                command_params_dict={
                    "fields": [
                        "debit_move_id",
                        "credit_move_id",
            ...
            for contador, reconcile in enumerate(reconcile_datas, start=1):
                ...
                if not debit_move:
                    message = f'El asiento de debito no se encuentra en el sistema (id: --> {old_debit_move_id})'
                    migrator.create_error_log(msg=message, values=reconcile)
                elif not debit_move.reconciled and debit_move.move_id.state == "draft":
                    message = f'El asiento de debito {debit_move.move_id.name} no se encuentra validado (id: --> {old_debit_move_id})'
                    migrator.create_error_log(msg=message, values=reconcile)
                # Credito
    ...
    def _get_migrator_for(self, migrator_type: str):
        migrators = {
            ...
            "product_product": self.migrate_product_template,
            ...
            "internal_transfer_payments": partial(self.migrate_account_payments, payment_type="internal_transfer"),
            #
            # Post
            ...
            "supplier_reconcile": partial(self.migrate_reconcile_moves, move_type="supplier"),
            "entry_reconcile": partial(self.migrate_reconcile_moves, move_type="entry"),
